...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val nBoundaries = new Boundaries

    nBoundaries.minX = Math.min(a.minX, b.minX)
    nBoundaries.maxX = Math.max(a.maxX, b.maxX)

    nBoundaries.minY = Math.min(a.minY, b.minY)
    nBoundaries.maxY = Math.max(a.maxY, b.maxY)
